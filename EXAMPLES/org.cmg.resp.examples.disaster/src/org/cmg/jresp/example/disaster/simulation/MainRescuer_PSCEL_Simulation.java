...
import org.cmg.jresp.examples.disaster.rescuer.pscel.Explorer;
import org.cmg.jresp.examples.disaster.rescuer.pscel.HelpRescuer;
import org.cmg.jresp.examples.disaster.rescuer.pscel.IsMoving;
import org.cmg.jresp.examples.disaster.rescuer.pscel.RandomWalk;
import org.cmg.jresp.examples.disaster.rescuer.pscel.Scenario;
import org.cmg.jresp.examples.disaster.rescuer.pscel.SpatialPanel;
 ...
 *
 */
public class MainRescuer_PSCEL_Simulation extends JFrame {
	...
	public MainRescuer_PSCEL_Simulation(int robots, int numSwarmRescuer, double height, double width) {
		super("Disaster scenario in jRESP by using the PSCEL language");
	...
	private void instantiateNet() {
		...
		for (int i = 0; i < scenario.getRobots(); i++) {
			...
			// starting robot role
			n.addAttribute(new Attribute("role", Scenario.EXPLORER));

			// n.addAttribute(new Attribute("name", "value"));
			// n.addAttribute(new Attribute("action", "stop"));
			...
			n.addAttributeCollector(new AttributeCollector("xRobot") {
				@Override
				protected Object doEval(Tuple... t) {
					return scenario.getPosition(robotIndex).getX();
				}
			});

			n.addAttributeCollector(new AttributeCollector("yRobot") {
				@Override
				protected Object doEval(Tuple... t) {
					return scenario.getPosition(robotIndex).getY();
				}
			});
			...
			// add Automaton Transitions // 1 - From Explorer [0] to HelpRescuer
			// [2]
			TargetTreeRepresentation cond_from0to2 = new TargetTreeRepresentation(TargetConnector.AND,
					new TargetTreeRepresentation(new TargetExpression(Equal.class,
							new RequestAttributeName("action", "id"), ActionThisID.UPD)),
			...
			TargetTreeRepresentation cond_from0to1 = new TargetTreeRepresentation(TargetConnector.AND,
					...
					new TargetTreeRepresentation(new TargetExpression(PatternMatch.class,
							...
							new Template(new ActualTemplateField("role"), new ActualTemplateField(Scenario.RESCUER)))));
			...
			TargetTreeRepresentation cond_from0to3 = new TargetTreeRepresentation(TargetConnector.AND,
					...
					new TargetTreeRepresentation(new TargetExpression(PatternMatch.class,
							new RequestAttributeName("action", "arg"), new Template(new ActualTemplateField("role"),
									new ActualTemplateField(Scenario.LOW_BATT)))));
	...
	public static void main(String[] argv) {
		...
		new MainRescuer_PSCEL_Simulation(numRobots, numRescuerSwarmSize, HEIGHT, WIDTH);
	}
