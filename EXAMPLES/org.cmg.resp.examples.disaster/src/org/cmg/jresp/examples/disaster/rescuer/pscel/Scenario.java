...
public class Scenario extends Observable {
	...
	public static final int victim_number = 2;
	...
	public static final double dechargedBattery = 30.0;
	...
	private static final int rechargingRobots = 35;
	...
	public void init() {
		...
		for (int i = 0; i < numberOfRobots; i++) {
			...
			if (i < (numberOfRobots * rechargingRobots / 100))
				robots[i] = new Robot(i, 2.5, addNoise(chargedBattery));
			else
				robots[i] = new Robot(i, 2.5, chargedBattery * 4);
	 ...
	 *            robot index
	 */
	public double getBattery(int i) {
		return robots[i].getBattery();
	}
		...
		for (int i = 0; i < numberOfRobots; i++) {
			if (robots[i].walking && !robots[i].getRole().equals(LOW_BATT)) {
				...
				if (robots[i].getRole().equals(LOW_BATT)) {
					if (robots[i].getUnderRecharging()) {
						if (robots[i].getRechargingSteps() < chargingSteps) {
							robots[i].addReachrgingStep();
							// System.out.println("Step" +
							// robots[i].getRechargingSteps() + " to go " +
							// (chargingSteps -
							// robots[i].getRechargingSteps()));
						} else {
							// ROBOT CHARGED
							robots[i].setUnderRecharging(false);
							robots[i].updateBatteryChargedSensor();
						}
					}
				}
			}
	...
	public class Robot {
					...
					for (int i = 0; i < victims.length; i++) {
						...
						if (victims[i].distance(this.position) <= VICTIM_SENSOR_RANGE
							...
							if (this.role.equals(Scenario.EXPLORER) && victim_anyrescuer[i]) { // rescuers[i]
								...
								return new Tuple("VICTIM_PERCEIVED", false, 0);
							}
							if (this.role.equals(Scenario.HELP_RES)) {
								// the help_rescuer is going to the victim
								return new Tuple("VICTIM_PERCEIVED", true, rescuers[i]);
							}
							...
							return new Tuple("VICTIM_PERCEIVED", true, 0);
		...
		private void updateVictimSensor() {
			if (!victimFound)
				victimSensor.setValue(detectVictim());
		}
		...
		private void decreseBatteryLevel(double dim) {
			...
			batteryChargedSensor.setValue(new Tuple("CHARGED", false));
		}
		...
		return new NodeConnection() {
			...
			public boolean areInTouch(String src, String target) {
				try {
					...
					// in order to avoid communication between (at least one)
					// robot in Low_Battery role
					if ((robots[idSrc].getRole().equals(Scenario.LOW_BATT)
							|| robots[idTrg].getRole().equals(Scenario.LOW_BATT))) {
						return false;
					}
					// no communications between two rescuers
					if ((robots[idSrc].getRole().equals(Scenario.RESCUER)
							&& robots[idTrg].getRole().equals(Scenario.RESCUER))) {
						return false;
					}
					// no communications from rescuer and help_rescuer
					if ((robots[idSrc].getRole().equals(Scenario.RESCUER)
							&& robots[idTrg].getRole().equals(Scenario.HELP_RES))) {
						return false;
					}
