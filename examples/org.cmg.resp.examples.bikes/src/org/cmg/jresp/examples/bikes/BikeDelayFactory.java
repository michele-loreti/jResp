 ...
 *
 */
public class BikeDelayFactory implements IDelayFactory {

	private Random random;

	public BikeDelayFactory( Random random ) {
		this.random = random;
	}
	...
	public double getGroupGetTime(SimulationNode src, Template t,
		...
		if (t == Scenario.RESERVE_BIKE) {
			Attribute a = trg.getAttribute("BIKES");
			if (a == null) {
				return -1;
			}
			Object o = a.getValue();
			...
			return sampleExponential(1.0*value*value);
		}
		if (t == Scenario.RESERVE_SLOT) {
			Attribute a = trg.getAttribute("SLOTS");
			if (a == null) {
				return -1;
			}
			Object o = a.getValue();
