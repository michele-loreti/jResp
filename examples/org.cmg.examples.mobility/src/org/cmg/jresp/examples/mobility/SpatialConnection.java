...
public class SpatialConnection extends Observable implements NodeConnection {
	...
	private void init(int size) {
		for (int i = 0; i < size; i++) {
			...
			this.locations.put(node,
					new RobotLocation(node, new Point2D.Double(maxX * r.nextDouble(), maxY * r.nextDouble())));
	...
	public AbstractActuator getDirectionActuator(final String n) {
		return new AbstractActuator("direction") {
			...
			public Template getTemplate() {
				return new Template(new ActualTemplateField("DIR"), new FormalTemplateField(Double.class));
			}
		};
	}
	...
	public class RobotLocation {
		...
		public RobotLocation(String name, Point2D.Double point) {
			...
			this.locationSensor = new AbstractSensor(name + "-LocationSensor",
					new Template(new ActualTemplateField("GPS"), new FormalTemplateField(Double.class),
		...
		public void setPoint(Point2D.Double point) {
			...
			if (point == null) {
				...
				locationSensor.setValue(new Tuple(("GPS"), (point.x), (point.y)));
				targetSensor.setValue(new Tuple("TARGET", point.distance(targetX, targetY) < targetSize / 2 - 5));
			}
		}
